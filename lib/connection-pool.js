...
function createConnection(pooled, attemptNumber) {
    ...
    var handleError = function(err) {
        ...
        var delay = self.retryDelay.forAttempt ? (self.retryDelay.forAttempt[attemptNumber] || self.retryDelay.default) : self.retryDelay;
        pooled.timeout = setTimeout(createConnection.bind(self, pooled, attemptNumber + 1), delay);
        self.emit('error', err);
    };
...
function fill() {
    ...
    for (i = 0; i < amount; i++)
        createConnection.call(this, null, 0);
}
